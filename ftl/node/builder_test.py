...
class NodeTest(unittest.TestCase):
    ...
    def test_create_package_base_uncached(self):
        ...
        ctx = context.Workspace(os.path.join(current_dir, "testdata/node_app"))
        with builder.Node(ctx) as b:
            ...
            with docker_image.FromDisk(base_config, zip(sha_list, tar_list)) as base_image:
                ...
                with b.CreatePackageBase(base_image, cash) as node_app_layer:
                    # check that the node_app_layer image was added to the cache
                    self.assertEqual(1, len(cash.GetMap()))
                    for k in cash.GetMap():
                        self.assertEqual(str(k).startswith("fake.gcr.io/google-appengine/node-package-lock-cache"),
                        True)
                cash.ResetCacheMiss()
                with b.CreatePackageBase(base_image, cash) as node_app_layer:
                    # check that the node_app_layer image was added to the cache
